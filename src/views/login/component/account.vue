...
export default defineComponent({
	...
	setup() {
		...
		const onSignIn = async () => {
			...
			// 存储 token 到浏览器缓存.substr(0)
			Session.set('token', Math.random().toString(36));
			// 存储用户信息到浏览器缓存
		...
		const signInSuccess = () => {
			...
			if (route.query?.redirect) {
				router.push({
					...
					query: route.query?.params && Object.keys(route.query?.params).length > 0 ? JSON.parse(route.query?.params as string) : '',
				});
			} else {
				router.push('/');
			}
